...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `Talent2flow will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "Talent2flow handles this" mechanics bullets, composed from the
